...
import QuestionBankView, { QBankReadAloudToolbar } from '@/components/QuestionBankView';
import type { ToolId } from '@/components/ToolsMenu';
import { getResourceContent, getFlashcards, DifficultyLevel } from '@/data/contentRepository';
...
import { startSpeechSession, hasWebSpeechSupport, type SpeechSession } from '@/services/speechService';

import { parseQuestionBank, type QuizQuestion } from '@/utils/quizParser';
import { trackChapterVisit, trackToolUsage, trackPersonaChange } from '@/utils/analytics';
import { useAudioProgress, type AudioProgress, GlobalAudioProgress } from '@/hooks/useAudioProgress';
...
export default function StudyTable() {
  ...
  const PERSONAS: { key: DifficultyLevel; label: string; imgSrc: string }[] = [
    ...
    { key: 'intermediate', label: 'Intermediate', imgSrc: '/personas/intermediate.png' },
  ...
  const chapterName = searchParams.get('chapterName') || `Chapter ${chapterNumber}`;
  ...
  const [autoResumeTrack, setAutoResumeTrack] = useState<'long' | 'short' | null>(null);
  ...
  const setActiveTool = (tool: ToolId) => {
    ...
    setSearchParams(prev => { const p = new URLSearchParams(prev); p.set('tool', tool); return p; }, { replace: true });
    trackToolUsage({ tool, subjectId, chapterNumber, chapterName: searchParams.get('chapterName') || `Chapter ${chapterNumber}` });
  };
  ...
  const railCollapseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const [readingHighlight, setReadingHighlight] = useState<{ active: boolean; startWord: number; endWord: number }>({
    active: false,
    startWord: -1,
    endWord: -1,
  });
  ...
  const NON_PERSONA_TOOLS = new Set(['pyq', 'swot', 'leaderboard', 'ask', 'videos', 'mindmap']);
  ...
  useEffect(() => {
    trackChapterVisit({
      ...
      subjectName,
      chapterNumber,
      chapterName: searchParams.get('chapterName') || `Chapter ${chapterNumber}`,
    });
  ...
  useEffect(() => {
    const checkAutoResume = async () => {
      ...
      if (progress && progress.currentTime > 0 && progress.subjectId === subjectId && progress.chapterNumber === chapterNumber && progress.persona === persona && progress.wasPaused) {
    ...
    checkAutoResume();
  }, [subjectId, chapterNumber, persona, navState?.autoPlayPodcast, toolFromUrl]);
  ...
  useEffect(() => {
    ...
    if (activeTool === 'summary') {
      getResourceContent(internalSubId, chapterNumber, 'summary.md', persona)
        ...
        .catch(() => { setContent('Summary content unavailable for this persona.'); setLoading(false); });
    } else if (activeTool === 'detailed') {
      getResourceContent(internalSubId, chapterNumber, 'detailed_view.md', persona)
        .then(res => { setContent(res); setLoading(false); })
        .catch(() => { setContent('Detailed notes unavailable.'); setLoading(false); });
      ...
      getResourceContent(internalSubId, chapterNumber, 'mindmap.md', persona)
        .then(res => { setContent(res); setLoading(false); })
        .catch(() => { setContent('graph TD\n  A[Unavailable] --> B[Mindmap not found]'); setLoading(false); });
    } else if (activeTool === 'qbank' || activeTool === 'popquiz') {
      getResourceContent(internalSubId, chapterNumber, 'question_bank.md', persona)
        .then(res => { setContent(res); setLoading(false); })
        .catch(() => { setContent('Question bank unavailable.'); setLoading(false); });
    } else if (activeTool === 'videos') {
      getResourceContent(internalSubId, chapterNumber, 'youtube_links.md', persona)
        .then(res => { setContent(res); setLoading(false); })
  ...
  const renderContent = () => {
    if (loading) return <div className="loading-state">Loading {activeTool}...</div>;
    ...
    switch (activeTool) {
      ...
      case 'mindmap':
        return <div className="mindmap-container"><MermaidView content={typeof content === 'string' ? content : ''} /></div>;
      case 'flashcards':
      ...
      case 'ask':
        return <AskBotView chapterName={chapterName} subjectId={subjectId} chapterNumber={chapterNumber} difficulty={persona} />;
      case 'podcasts':
        return <PodcastsView chapterName={chapterName} subjectId={subjectId} chapterNumber={chapterNumber} persona={persona} autoPlay={navState?.autoPlayPodcast} resumeTrack={navState?.resumeTrack || autoResumeTrack} onPlayingChange={setPodcastIsPlaying} playTrigger={podcastPlayTrigger} pauseTrigger={podcastPauseTrigger} />;
      case 'videos':
        return <VideosView chapterName={chapterName} subjectId={subjectId} chapterNumber={chapterNumber} youtubeLinksContent={typeof content === 'string' ? content : null} />;
      case 'pyq':
  ...
  const TOOL_LABELS: Record<string, string> = {
    summary: 'Study Table', detailed: 'Study Table', mindmap: 'Mindmap',
    flashcards: 'Flash Cards', podcasts: 'Podcasts', videos: 'Videos',
    ask: 'Ask', popquiz: 'Quiz', qbank: 'Question Bank',
    swot: 'SWOT Analysis', pyq: 'PYQ', leaderboard: 'Leaderboard'
  };

  return (
    <div className="sv-app st-app" style={{ display: 'flex', flexDirection: 'row', height: '100vh', overflow: 'hidden' }}>
      {/* Mobile overlay */}
      {isSidebarOpen && window.innerWidth <= 992 && (
        <div className="sv-mobile-overlay" onClick={() => setIsSidebarOpen(false)}></div>
      )}

      {/* ── DARK RAIL (BACK ON LEFT) ── */}
      <nav
        className={`sv-rail ${!isSidebarOpen ? 'collapsed' : ''}`}
      >
        <button 
        ...
        <div className="sv-rail-items" style={{ overflowY: 'auto', flex: 1, paddingBottom: '20px' }}>
          {[
      ...
      {!isSidebarOpen && (
        <div
          ...
          style={{ position: 'fixed', left: 0, top: 0, width: '14px', height: '100vh', zIndex: 65, cursor: 'pointer' }}
        />
      )}

      {/* ── MAIN ── */}
      <main className="sv-main book-main" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Frosted topbar */}
        <header className="sv-topbar">
          <div className="sv-topbar-left">
            <div className="sv-rail-brand" onClick={() => navigate('/')} style={{ cursor: 'pointer', marginRight: '16px', display: 'flex', alignItems: 'center' }}>
              <img className="sv-brand-logo-img sv-study-rail-tree-logo" src={treeLogoSrc} alt="Saral Vidhya" />
            </div>
            <div className="sv-breadcrumb">
              <span className="sv-bc-link" onClick={handleBackToClass}>{className || 'Class'}</span>
              <span className="sv-bc-sep">/</span>
              <span className="sv-bc-link" onClick={handleBackToChapters}>{subjectName || 'Subject'}</span>
              <span className="sv-bc-sep">/</span>
              <span className="sv-bc-link" onClick={handleBackToChapters}>Chapters</span>
              <span className="sv-bc-sep">/</span>
          ...
          <div className="st-body" style={{ flex: 1, overflowY: activeTool === 'flashcards' || activeTool === 'podcasts' ? 'hidden' : 'auto', padding: activeTool === 'flashcards' || activeTool === 'popquiz' || activeTool === 'podcasts' ? '8px 20px 0 20px' : '16px 20px', display: 'flex', flexDirection: 'column', minHeight: 0, background: 'transparent' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: '20px', marginBottom: activeTool === 'flashcards' || activeTool === 'popquiz' ? '6px' : '12px', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <h1 className="st-page-title" style={{ margin: 0, fontSize: '1.6rem' }}>
                    {['summary', 'detailed'].includes(activeTool) ? 'Study Table' : TOOL_LABELS[activeTool]}
                  </h1>
                  {['summary', 'detailed'].includes(activeTool) && (
                    <div className="st-page-subtitle-row">
                      <span className="st-page-subtitle">{activeTool === 'summary' ? 'Summary' : 'Detailed view'}</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                {showPersonaSwitcher && (
                  <div className="persona-switcher persona-switcher--icons" style={{ margin: 0, padding: '4px 8px', gap: '8px', flexShrink: 0 }}>
                    {PERSONAS.map(p => (
                      <button
                        key={p.key}
                        type="button"
                        ...
                        style={{ 
                          ...
                          overflow: 'hidden',
                          borderRadius: '16px',
                          border: persona === p.key ? '3px solid #7C3AED' : '2px solid rgba(0, 0, 0, 0.08)',
                          boxShadow: persona === p.key ? '0 4px 12px rgba(124, 58, 237, 0.2)' : '0 2px 6px rgba(0,0,0,0.05)',
                          background: '#ffffff',
                          transition: 'all 0.2s ease'
                        }}
                      >
                        <img src={p.imgSrc} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                      </button>
                    ))}
                  </div>
                )}
                ...
                {activeTool === 'flashcards' && (
                  <div className="read-aloud-tool-pill read-aloud-tool-pill--transport">
                    <div className="read-aloud-play-cluster">
                      <button
                        type="button"
                        onClick={() => { setFlashcardSpeakTrigger(t => t + 1); setIsSidebarOpen(false); }}
                        className="read-aloud-play-btn"
                        aria-label={flashcardIsSpeaking ? 'Pause reading' : 'Read aloud'}
                        data-label={flashcardIsSpeaking ? 'Pause' : 'Read aloud'}
                      >
                        {flashcardIsSpeaking ? (
                ...
                {activeTool === 'podcasts' && (
                  <div className="read-aloud-tool-pill read-aloud-tool-pill--transport">
                    <div className="read-aloud-play-cluster">
                      <button
                        type="button"
                        onClick={() => { podcastIsPlaying ? setPodcastPauseTrigger(t => t + 1) : setPodcastPlayTrigger(t => t + 1); if (!podcastIsPlaying) setIsSidebarOpen(false); }}
                        className="read-aloud-play-btn"
...
function ReadAloudBar({
  ...
  const handlePlayPause = () => {
    ...
    if (playbackRef.current === 'playing') {
      ...
      const clean = stripMarkdown(text);
      if (!clean) { cooldownRef.current = false; return; }
      cleanTextRef.current = clean;
      const savedProgress = Number(localStorage.getItem(progressStorageKey) || '0');
      const startChar = Number.isFinite(savedProgress) ? Math.max(0, Math.min(clean.length - 1, savedProgress)) : 0;
  ...
  return (
    <div className="read-aloud-toolbar-row">
      ...
      >
        {onSwitchTool && (activeTool === 'summary' || activeTool === 'detailed') && (
          <div className="read-aloud-tool-pill read-aloud-tool-pill--view">
            <button
              type="button"
              className="read-aloud-view-toggle-btn"
              data-label={activeTool === 'summary' ? 'Detailed view' : 'Summary'}
              aria-label={activeTool === 'summary' ? 'Switch to detailed view' : 'Switch to summary'}
              onClick={() => onSwitchTool(activeTool === 'summary' ? 'detailed' : 'summary')}
            >
              {activeTool === 'summary' ? (
        ...
        <div className="read-aloud-tool-pill read-aloud-tool-pill--transport">
          <div className="read-aloud-play-cluster">
            <button
              ...
              className="read-aloud-play-btn"
              type="button"
              aria-label={playbackState === 'playing' ? 'Pause reading' : 'Play reading'}
            ...
            >
              {playbackState === 'playing' ? (
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style={{ marginLeft: playbackState === 'paused' ? '0' : '2px' }}>
                  <path d="M8 5v14l11-7z"/>
                </svg>
              )}
            </button>
...
function AskBotView({ chapterName, subjectId, chapterNumber: _chapterNumber, difficulty: _difficulty }: { chapterName: string; subjectId: string; chapterNumber: number; difficulty: DifficultyLevel }) {
  const chatStorageKey = useMemo(() => `ask_chat_${subjectId}_${_chapterNumber}_${_difficulty}`, [subjectId, _chapterNumber, _difficulty]);
  const draftStorageKey = useMemo(() => `ask_draft_${subjectId}_${_chapterNumber}_${_difficulty}`, [subjectId, _chapterNumber, _difficulty]);
  const [messages, setMessages] = useState<{ role: 'user' | 'bot'; text: string; loading?: boolean }[]>(() => {
    try {
      const raw = localStorage.getItem(chatStorageKey);
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed.filter((m: any) => m && typeof m.text === 'string' && (m.role === 'user' || m.role === 'bot')).slice(-30) : [];
    } catch {
      return [];
    }
  });
  const [input, setInput] = useState(() => localStorage.getItem(draftStorageKey) || '');
  const [isSending, setIsSending] = useState(false);
  const [isListening, setIsListening] = useState(false);
  const [isTranscribing, setIsTranscribing] = useState(false);
  const [speakingIdx, setSpeakingIdx] = useState<number | null>(null);
  const [selectedLang, setSelectedLang] = useState(subjectId === 'pubadm_ur' ? 'ur-PK' : 'en-IN');
  ...
  const startMic = async () => {
    ...
    try {
      ...
      setIsListening(true);
      console.log('Microphone started successfully, streaming:', session.isStreaming);
    } catch (e: any) {
      console.error('Could not start speech session:', e);
      const errMsg = e.message || 'Could not start microphone. Please check permissions and try again.';
      setMicError(errMsg);
      setIsListening(false);
  ...
  const stopMic = async (autoSend = false) => {
    ...
    try {
      ...
      const errMsg = e.message || 'Could not transcribe audio. Please try again.';
      setMicError(errMsg);
    ...
    const finalTranscript = transcript || (isStreamingMic ? currentInputRef.current : '');
  ...
  return (
    <div className="ask-bot-view" role="region" aria-label="Ask AI Assistant" style={{ position: 'relative' }}>
      ...
      <div className="ask-header">
        <div className="ask-header-row">
          <div>
            {!isStreamingMic && (
              <p style={{ fontSize: '0.85em', color: '#666', marginTop: '4px' }}>
                🦊 Firefox Mode: Audio will be transcribed when you stop recording
              </p>
            )}
          </div>
          ...
          <div className="lang-selector">
            ...
            <select id="ask-lang" value={selectedLang} onChange={e => setSelectedLang(e.target.value)}>
              {ASK_LANGUAGES.map(l => (
      ...
      <div className="ask-chat-window" role="log" aria-live="polite" aria-label="Chat messages">
        {messages.length === 0 && (
          <div className="chat-placeholder">
            <p>👋 Hi there! I'm Saral Vidhya, your academic tutor for NCERT Class 10 English. We're currently looking at "A Letter to God."</p>
            <p style={{ fontSize: '0.85em', opacity: 0.7, marginTop: '0.5rem' }}>How can I help you today? Do you have any questions about the chapter, its themes, characters, or anything else related to your studies? Feel free to ask! 😊</p>
          </div>
        )}
        {messages.map((m, i) => (
          <div key={i} className={`chat-bubble ${m.role}`} role="article" aria-label={m.role === 'user' ? 'Your question' : 'AI response'}>
            {m.loading ? (
              <div className="chat-text" style={{ fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }} aria-label="AI is thinking">
                <span className="gear-spin" style={{ display: 'inline-block', fontSize: '1.8rem' }}>⚙️</span> <span style={{ opacity: 0.8 }}>Generating answer...</span>
              </div>
            ) : m.role === 'bot' ? (
              <>
                ...
                <div className="tts-controls">
                  {speakingIdx !== i ? (
                    ...
                    <button className="btn-tts speaking" onClick={stopSpeech} title="Stop reading" aria-label="Stop reading">⏹</button>
                  )}
                </div>
              </>
            ) : (
              <div className="chat-text" dir="auto">{m.text}</div>
            )}
          </div>
        ))}
        <div ref={chatEndRef} />
      ...
      {(isListening || isTranscribing) && (
        <div className="listening-indicator" aria-live="assertive">
          {isTranscribing ? (
            <><span className="pulse-dot transcribing"></span> Transcribing your speech… please wait</>
          ) : isStreamingMic ? (
            <><span className="pulse-dot"></span> Listening… speak now, then click ⏹ to stop &amp; send</>
          ) : (
            <><span className="pulse-dot recording"></span> Recording… click ⏹ to stop and transcribe (works in all browsers)</>
          )}
        </div>
      )}
    </div>
...
function PodcastsView({ chapterName, subjectId, chapterNumber, persona, autoPlay, resumeTrack, onPlayingChange, playTrigger, pauseTrigger }: { chapterName: string; subjectId: string; chapterNumber: number; persona: string; autoPlay?: boolean; resumeTrack?: string; onPlayingChange?: (playing: boolean) => void; playTrigger?: number; pauseTrigger?: number; }) {
  ...
  const getSavedTrack = (): 'long' | 'short' => {
    ...
    const saved = localStorage.getItem(`last_track_${subjectId}_${chapterNumber}_${persona}`);
    return (saved === 'long' || saved === 'short') ? saved : 'short';
  };

  const [selectedTrack, setSelectedTrack] = useState<'long' | 'short'>(getSavedTrack());

  useEffect(() => {
    localStorage.setItem(`last_track_${subjectId}_${chapterNumber}_${persona}`, selectedTrack);
  ...
  const wordTimings = useMemo(() => {
    if (!transcriptTokens.length) return { totalWeight: 0, wordWeights: [] as number[], wordIndexes: [] as number[] };
  ...
  const { activeWordIdx, activeWordSet, activeTokenStart, activeTokenEnd } = useMemo(() => {
    if (!transcript || !audioDuration || wordTimings.wordIndexes.length === 0 || currentTime < 0) {
      return { activeWordIdx: -1, activeWordSet: new Set<number>(), activeTokenStart: -1, activeTokenEnd: -1 };
    }
    ...
    const t2 = wordTimings.wordIndexes[Math.min(wordIdx + 1, wordTimings.wordIndexes.length - 1)];
    const t3 = wordTimings.wordIndexes[Math.min(wordIdx + 2, wordTimings.wordIndexes.length - 1)];
  ...
  useEffect(() => {
    ...
    const phraseActive = transcriptBoxRef.current.querySelector('.transcript-phrase-active');
    if (phraseActive) {
      targetElement = phraseActive as HTMLElement;
    } else {
      // Fall back to word highlighting
      const activeWords = transcriptBoxRef.current.querySelectorAll('.transcript-word.active');
  ...
  useAudioProgress(userId, lessonId, audioRef, available === true, metadata, setCurrentTime);

  useEffect(() => {
    const audio = audioRef.current;
    if (!audio || available !== true) return;

    const savedData = localStorage.getItem(`audio_progress_${userId}_${lessonId}`);
    if (!savedData) return;
  ...
  const getFileNames = (track: 'long' | 'short') => {
    if (subjectId === 'pubadm_ur') {
      return {
        audio: track === 'long' ? `${basePath}/podcast_long.m4a` : `${basePath}/podcast_short.mp3`,
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) { setTranscriptVisible(true); observer.disconnect(); } },
      { threshold: 0.1 }
    );
    observer.observe(el);
  ...
  const fmtTime = (s: number, fallback = '0:00') => {
    if (s === undefined || s === null || isNaN(s) || !isFinite(s)) return fallback;
  ...
  // ── Loading state ──
  if (available === null) {
    return <div className="podcasts-view"><p className="loading-state">Checking podcast availability…</p></div>;
  }
    ...
    return (
      <div className="podcasts-view">
        <div className="card" style={{ padding: '48px', textAlign: 'center', marginTop: '20px' }}>
          ...
          <h3 style={{ margin: '16px 0 8px', color: 'var(--text)' }}>Coming Soon</h3>
          <p style={{ color: 'var(--text-secondary)' }}>
            Podcasts for <strong>{chapterName}</strong> haven't been uploaded yet.<br />Check back soon!
          </p>
        </div>
      </div>
    );
    ...
    <div className="podcasts-view">
        ...
        <div className="podcast-player-real">
          ...
          <div className="player-row">
            ...
            <div className="player-bar-wrap">
              <div className="player-bar-fill" style={{ width: `${progressPct}%` }} />
              <input
                type="range"
                min={0}
                max={audioDuration || 0}
            ...
            <span className="player-time total">{audioDuration ? fmtTime(audioDuration) : '--:--'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="podcast-transcript-scroll" ref={transcriptRef}>
        <div className="transcript-toggle">
          <h4>Transcript</h4>
          {transcript && (
            <button className="btn-transcript-toggle" onClick={() => setShowTranscript(!showTranscript)}>
              {showTranscript ? 'Hide' : 'Show'}
            </button>
          )}
        </div>
        {!transcriptVisible && (
          <div style={{ height: '48px', display: 'flex', alignItems: 'center', color: 'var(--text-secondary)', fontSize: '0.85rem', opacity: 0.6 }}>
            Scroll down to load transcript…
          </div>
        )}
        {transcriptVisible && transcript === null && (
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>No transcript available.</p>
        )}
        {transcriptVisible && showTranscript && transcript && (
          <div className="transcript-box transcript-body" ref={transcriptBoxRef}>
            {transcriptTokens.map((token, idx) => {
              if (activeTokenStart >= 0 && idx === activeTokenStart) {
                const grouped = transcriptTokens.slice(activeTokenStart, activeTokenEnd + 1).join('');
                return <span key={`grp-${idx}`} className="transcript-phrase-active">{grouped}</span>;
              }
              if (activeTokenStart >= 0 && idx > activeTokenStart && idx <= activeTokenEnd) return null;
              const isWord = token.trim() !== '' && !/^[.,!?;:]+$/.test(token);
              return <span key={idx} className={isWord && activeWordSet.has(idx) ? 'transcript-word active' : undefined}>{token}</span>;
            })}
          </div>
        )}
      </div>
...
function PYQView({ subjectName, subjectId }: { subjectName: string; subjectId: string }) {
  ...
  if (subjectId === 'pubadm_ur') {
    ...
    pdfUrl = selectedYear === '2024' ? `/generated_resources/science/pyq/2024/31_2_2_Science.pdf` : `/generated_resources/science/pyq/2025/31-6-1_Science.pdf`;
  } else if (subjectId === 'english') {
    pdfUrl = selectedYear === '2024' ? `/generated_resources/english/pyq/2024/2_3_2_English%20L%20&%20L.pdf` : `/generated_resources/english/pyq/2025/2-1-1_English%20Language%20and%20Literature.pdf`;
  }
    ...
    return (
      <div className="pyq-view">
        <div className="card" style={{ padding: '40px', textAlign: 'center', marginTop: '20px' }}>
          ...
          <p style={{ marginTop: '1rem' }}>No previous year question papers available for this subject.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pyq-view">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <div className="pyq-year-selector" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        ...
        <div className="pyq-controls" style={{ display: 'flex', gap: '8px' }}>
          <select disabled style={{ padding: '8px 16px', borderRadius: '12px', background: '#f8fafc', border: '1.5px solid #e2e8f0', fontSize: '0.85rem', fontWeight: 600, color: '#64748b' }}>
            <option>{subjectName}</option>
          </select>
        </div>
      </div>

      <div className="pdf-viewer card" style={{ height: 'calc(100vh - 220px)', minHeight: '500px', padding: 0, overflow: 'hidden', borderRadius: '12px' }}>
        {isPdfAvailable === null ? (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <p className="opacity-70" style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>Checking document availability...</p>
          </div>
        ) : isPdfAvailable ? (
          <iframe
          ...
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)' }}>
            <span style={{ fontSize: '4rem', opacity: 0.4, marginBottom: '16px' }}>📄🚫</span>
            <h3 style={{ margin: 0, color: 'var(--text)', fontSize: '1.4rem' }}>No Document Available</h3>
            <p style={{ color: 'var(--text-secondary)', opacity: 0.8, marginTop: '8px', fontWeight: 500 }}>
              The question paper PDF for the year {selectedYear} hasn't been uploaded yet.
            </p>
          </div>
        )}
      </div>
...
function VideosView({ chapterName: _chapterName, subjectId, chapterNumber, youtubeLinksContent }: { chapterName: string; subjectId: string; chapterNumber: number; youtubeLinksContent?: string | null }) {
  ...
  if (videos.length > 0) {
    ...
    return (
      <div className="videos-view" style={{ display: 'flex', gap: '12px', height: '100%', alignItems: 'flex-start' }}>
          ...
          {videos.map((vid, i) => (
            <div
              ...
              style={{
                cursor: 'pointer',
                padding: '6px',
                border: robustActive === i ? '2px solid var(--primary)' : '2px solid transparent',
                borderRadius: '8px',
              }}
            >
              <div
              ...
              >
                <img
                  ...
                  alt="thumbnail"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: robustActive === i ? 1 : 0.7 }}
                />
              </div>
              <p style={{ fontSize: '0.7rem', fontWeight: 500, margin: 0, lineHeight: 1.3, wordBreak: 'break-word' }}>{vid.title}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div
            ...
            style={{ position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', borderRadius: '12px', background: '#000' }}
          >
            <iframe
  ...
  if (youtubeLinksContent && youtubeLinksContent !== 'Content not available.') {
    return (
      <div className="videos-view">
        <div className="card" style={{ padding: '4px 8px', marginBottom: '16px', background: 'var(--primary-light)', borderRadius: '10px', borderLeft: '4px solid var(--primary)' }}>
          <p style={{ margin: '8px 0', fontSize: '0.9rem', color: 'var(--primary)', fontWeight: 600 }}>
            🎥 No embedded videos yet — use these curated search queries on YouTube to find the best lessons for this chapter.
          </p>
        </div>
        <div className="markdown-container">
  ...
  return (
    <div className="videos-view">
      <div className="card" style={{ padding: '40px', textAlign: 'center', marginTop: '20px' }}>
        <span style={{ fontSize: '3rem', opacity: 0.5 }}>🎥</span>
        <p style={{ marginTop: '1rem' }}>No video content is currently available for this chapter.</p>
      </div>
    </div>
  );
}
...
function PopQuizView({ markdownContent, subjectId, chapterNumber, onQuit }: { markdownContent: string; subjectId: string; chapterNumber: number; onQuit: () => void }) {
  ...
  }, [step, selected, isFinished, loading, showQuitModal, showExplanation, questions.length]);
  ...
  const handleSelect = (idx: number) => {
    ...
    setSelected(idx);
    setAnswered(prev => { const next = [...prev]; next[step] = true; return next; });
    setHoveredOption(null);
  ...
  return (
    <>
      ...
      {loading || questions.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '24px', color: '#64748b' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '20px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: '800', color: '#1e1b4b' }}>Quiz Complete!</div>
      ...
      {showQuitModal && (
        <div className="quiz-quit-overlay">
          <div className="quiz-quit-modal card">
            <h3 style={{ margin: '0 0 16px', fontSize: '1.3rem' }}>Quit Quiz?</h3>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', margin: '16px 0 0', textAlign: 'center' }}>
              Your progress so far will not be saved.
            </p>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'center' }}>
              <button className="quiz-btn-secondary" onClick={() => setShowQuitModal(false)}>Continue Quiz</button>
              <button className="quiz-btn-danger" onClick={onQuit}>Quit &amp; Exit</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Wrong answer popup */}
      {showExplanation && selected !== null && selected !== questions[step].answer && (
        <div className="quiz-quit-overlay" onClick={() => setShowExplanation(false)}>
          <div className="quiz-explanation-modal card" onClick={(e) => e.stopPropagation()}>
            <button className="quiz-modal-close" onClick={() => setShowExplanation(false)} type="button" aria-label="Close">×</button>
            <h4 className="wrong-heading" style={{ margin: '0 0 14px 0' }}>❌ Incorrect</h4>
            <p style={{ marginBottom: '10px', margin: '0 0 10px 0' }}>
              <strong>Correct Answer:</strong>{' '}
              <span style={{ color: '#16a34a', fontWeight: 700 }}>{questions[step]?.options[questions[step]?.answer] || ''}</span>
            </p>
            <p style={{ color: '#334155', lineHeight: 1.65, margin: 0, fontSize: '0.95rem' }}>
              {questions[step]?.explanation && questions[step].explanation !== 'No explanation provided.'
                ...
                : (() => {
                    ...
                    const ans = questions[step]?.options[questions[step]?.answer] || '';
            ...
            <div style={{ display: 'flex', marginTop: '20px' }}>
              <button className="quiz-btn-primary" onClick={() => { setShowExplanation(false); handleNext(); }} style={{ flex: 1 }}>
                Next Question ➔
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: '12px', width: '100%', maxHeight: '100%' }}>
        ...
        <div className="quiz-nav" style={{ 
          ...
          <button className="quiz-icon-btn previous" onClick={handlePrevious} disabled={step === 0} title="Previous" type="button">←</button>
          <button className="quiz-icon-btn skip" onClick={handleSkip} title="Skip" type="button" style={{ fontSize: '1rem', width: '44px', height: '44px' }}>⏭</button>
          <span style={{ fontSize: '1rem', fontWeight: 700, color: '#8b5cf6', minWidth: '44px', textAlign: 'center' }}>
            {step + 1}/{questions.length}
          </span>
          <button className="quiz-icon-btn next" onClick={handleNext} disabled={step === questions.length - 1 && selected === null} title="Next" type="button">→</button>
        </div>
        </div>
        ...
        <button
          ...
          title="Quit Quiz"
          type="button"
          style={{ width: '40px', height: '40px', fontSize: '1rem', flexShrink: 0, marginTop: '4px' }}
        >
          ❌
        </button>
      </div>
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= maxChapters; i++) {
      ...
      if (timeSpent > 0 && timeSpent < 10 && score !== null) {
        threats.push(`${chapterName} completed suspiciously fast (${timeSpent}s). Possible skimming evasion.`);
      }
    }
    ...
    <div className="swot-view" style={{ padding: '20px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
        <div className="card swot-card">
          ...
          {swot.s.length > 0 ? <ul>{swot.s.map((c, i) => <li key={i}>{c}</li>)}</ul> : <p className="opacity-70">Read more chapters and take quizzes.</p>}
        </div>
        <div className="card swot-card">
          <h3 style={{ color: '#ff9800' }}>⚠️ Weaknesses</h3>
          {swot.w.length > 0 ? <ul>{swot.w.map((c, i) => <li key={i}>{c}</li>)}</ul> : <p className="opacity-70">No weaknesses yet! Great job.</p>}
        </div>
        <div className="card swot-card">
          <h3 style={{ color: '#2196f3' }}>🚀 Opportunities</h3>
          {swot.o.length > 0 ? <ul>{swot.o.map((c, i) => <li key={i}>{c}</li>)}</ul> : <p className="opacity-70">You've covered all the curriculum!</p>}
        </div>
        <div className="card swot-card">
          <h3 style={{ color: '#f44336' }}>🚨 Threats</h3>
          {swot.t.length > 0 ? <ul>{swot.t.map((c, i) => <li key={i}>{c}</li>)}</ul> : <p className="opacity-70">Your study patterns look healthy.</p>}
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {users.map((u, i) => (
          <div key={i} style={{
            ...
            border: u.isMe ? '2px solid var(--primary)' : '1px solid var(--border)',
            borderRadius: '10px',
            padding: '10px 16px',
            fontWeight: u.isMe ? 700 : 400,
          }}>
            <span style={{ width: '24px', fontWeight: 700, color: i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : i === 2 ? '#CD7F32' : 'var(--text-secondary)' }}>
              {i + 1}
            </span>
            <span style={{ flex: 1 }}>{u.name}{u.isMe ? ' (You)' : ''}</span>
            <span style={{ color: 'var(--primary)', fontWeight: 700 }}>{u.points} pts</span>
          </div>
        ))}
      </div>
    </div>
